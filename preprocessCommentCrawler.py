...
def main():
    ...
    for filename in os.listdir("commentCrawlerOutput/"):
        with open("commentCrawlerOutput/" + filename) as file:
            currentLine = []
            # Create output file
            # o = open("commentCrawlerOutputPreprocessed/" + filename, "w+")
            ...
            if "M" in filename:
                male_data.append([filename, finalTokens])
            elif "F" in filename:
                female_data.append([filename, finalTokens])
            else:
    ...
    for file in female_data:
        ...
        if filename in os.listdir("commentCrawlerOutputPreprocessed/"):
    ...
    # There are more male files than female files
    # Insert male files evenly across ratings
    step_size = int(len(male_data) / len(female_data)) + 1
    count = 0
    for index in range(0, len(male_data), step_size):
        if count == len(female_data):
            break

        filename, tokens = male_data[index]
    ...
    for index in range(0, len(male_data), int(len(male_data) / (len(male_data) - count))):
        ...
        filename, token = male_data[index]

        while filename in os.listdir("commentCrawlerOutputPreprocessed/"):
            index += 1
            filename, token = male_data[index]

        if filename in os.listdir("commentCrawlerOutputPreprocessed/"):
            print("Duplicate", index, filename)
    ...
    # Check number of male and female files
    num_male = 0
    num_female = 0

    for filename in os.listdir("commentCrawlerOutputPreprocessed/"):
        if "M" in filename:
            num_male += 1
        elif "F" in filename:
            num_female += 1
    ...
    print("Number of male files: ", num_male, len(male_data))
    print("Number of female files: ", num_female, len(female_data))
